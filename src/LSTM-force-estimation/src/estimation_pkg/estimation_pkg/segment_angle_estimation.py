...
class SegmentEstimationNode(Node):
  def __init__(self, roi_config_file='config_ROI_ref.json'):
    ...
    self.segment_angle_image_binary = Image()
    self.segment_angle_image_binary_publisher = self.create_publisher(
      Image,
      'estimated_segment_angle_image_binary',
      QOS_RKL10V
    )
    ...
    while rclpy.ok():
      if self.current_frame_flag:
        try:
          ...
          landmark_image_binary = self.rbsc.draw_arrows(self.rbsc.body_image)
          landmark_image_binary_msg = self.br_rgb.cv2_to_imgmsg(landmark_image_binary, 'bgr8')
          self.segment_angle_image_publisher.publish(landmark_image_binary_msg)
