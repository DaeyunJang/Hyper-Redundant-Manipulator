...
class SegmentEstimationNode(Node):
    ...
    def publish_segment_angles(
        ...
        if not self.published_first_segment_angle:
            self.get_logger().info(
                'Publishing estimated_segment_angle with 18-joint pan/tilt '
    ...
    def make_reconstruction_markers(
      ...
      segment_directions_filtered_xyz):
        ...
        center_points.scale.x = 0.0030
        center_points.scale.y = 0.0030
        center_points.scale.z = 0.0030
        ...
        segment_center_tangents_xyz,
            ...
            tangent = Marker()
            tangent.header = header
            tangent.ns = 'segment_center_tangents'
            tangent.id = 200 + index
            tangent.type = Marker.ARROW
            tangent.action = Marker.ADD
            tangent.pose.orientation.w = 1.0
            tangent.scale.x = 0.0007
        ...
        direction_arrow_length = 0.003
        direction_marker_sets = (
            (
        ...
        for namespace, id_offset, color, directions in direction_marker_sets:
            for index, (start, direction) in enumerate(zip(
                    segment_center_points_xyz, directions)):
                direction_marker = Marker()
                direction_marker.header = header
                direction_marker.ns = namespace
                direction_marker.id = id_offset + index
                direction_marker.type = Marker.ARROW
                direction_marker.action = Marker.ADD
                direction_marker.pose.orientation.w = 1.0
                direction_marker.scale.x = 0.0007
                direction_marker.scale.y = 0.0016
                direction_marker.scale.z = 0.0020
                direction_marker.color.r = color[0]
                direction_marker.color.g = color[1]
                direction_marker.color.b = color[2]
                direction_marker.color.a = 1.0
                direction_marker.points = [
                    self.xyz_to_point(start),
                    self.xyz_to_point(
                        start + direction_arrow_length * direction
                    ),
                ]
